...
namespace SH_jvyjhs_StudentExamScore_epost
{
    public class Program
    {
        ...
        {
            var btn = K12.Presentation.NLDPanels.Student.RibbonBarItems["資料統計"]["報表"]["成績相關報表"]["定期評量成績單-ePost"];
            btn.Enable = false;
            ...
            permission.Add(new RibbonFeature(Permissions.SH_jvyjhs_StudentExamScore_epost, "定期評量成績單-ePost"));
        }
        ...
        static void Program_Click(object sender_, EventArgs e_)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                {
                    ...
                    table.Columns.Add("成績" + subjectIndex);
                ...
                bkw.DoWork += delegate(object sender, System.ComponentModel.DoWorkEventArgs e)
                {
                    ...
                    //2016/10/24 穎驊筆記 清除暫存，始之重新抓取...，但發現沒有作用QQ 看來是更底層的東西，之後有時間再改，如果使用者有在操作時更動類別，然後要印成績單，
                    //建議要重新啟動系統，要不然會抓不到
                    //accessHelper.Reflash();
                    ...
                    try
                    {
                        ...
                        _dtEpost.Columns.Add("POSTALADDRESS");
                        _dtEpost.Columns.Add("POSTALCODE");
                        _dtEpost.Columns.Add("次別");
                        _dtEpost.Columns.Add("班級簡稱");
                        _dtEpost.Columns.Add("座號");
                        _dtEpost.Columns.Add("學號");
                        //_dtEpost.Columns.Add("導師姓名");
                        _dtEpost.Columns.Add("姓名");
                        _dtEpost.Columns.Add("學年度");
                        _dtEpost.Columns.Add("學期別");
                        ...
                        for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                        {
                            ...
                            //_dtEpost.Columns.Add("成績" + subjectIndex);
                            //_dtEpost.Columns.Add("班級平均" + subjectIndex);
                            _dtEpost.Columns.Add("班排名" + subjectIndex);
                            //_dtEpost.Columns.Add("科排名" + subjectIndex);
                            //_dtEpost.Columns.Add("類組排名" + subjectIndex);
                        }
                        // 處理科目相關成績
                        for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                        {
                            ...
                            //_dtEpost.Columns.Add("前次成績" + subjectIndex);
                            //_dtEpost.Columns.Add("成績" + subjectIndex);
                            _dtEpost.Columns.Add("班平均" + subjectIndex);
                            //_dtEpost.Columns.Add("班級排名" + subjectIndex);
                            //_dtEpost.Columns.Add("科排名" + subjectIndex);
                            //_dtEpost.Columns.Add("類組排名" + subjectIndex);
                        }

                        _dtEpost.Columns.Add("成績總分a");
                        _dtEpost.Columns.Add("成績a");
                        _dtEpost.Columns.Add("全班名次");
                        ...
                        eKeyValDict.Add("收件人", "CN");
                        eKeyValDict.Add("學年度", "學年度");
                        eKeyValDict.Add("學期別", "學期別");
                        eKeyValDict.Add("次別", "次別");
                        eKeyValDict.Add("班級簡稱", "班級簡稱");
                        //eKeyValDict.Add("班導師", "導師姓名");
                        eKeyValDict.Add("學號", "學號");
                        eKeyValDict.Add("座號", "座號");
                        eKeyValDict.Add("姓名", "姓名");
                        eKeyValDict.Add("成績總分a", "成績總分a");
                        eKeyValDict.Add("成績a", "成績a");
                        //eKeyValDict.Add("加權平均班均標", "班級加權平均");
                        eKeyValDict.Add("全班名次", "全班名次");
                        //eKeyValDict.Add("加權總分科排名", "加權總分科排名");
                        //eKeyValDict.Add("加權平均科均標", "科加權平均");
                        //eKeyValDict.Add("類別1加權總分排名", "加權總分類組排名");
                        //eKeyValDict.Add("類別1加權平均排名", "加權平均類組排名");
                        ...
                        foreach (var stuRec in studentRecords)
                        {
                            ...
                            row["學生類別"] = stuRec.StudentCategorys;
                            ...
                            XmlElement scoreCalcRule = ScoreCalcRule.Instance.GetStudentScoreCalcRuleInfo(stuRec.StudentID) == null ? null : ScoreCalcRule.Instance.GetStudentScoreCalcRuleInfo(stuRec.StudentID).ScoreCalcRuleElement;

                            DSXmlHelper helper = new DSXmlHelper(scoreCalcRule);

                            //K12.Data.StudentRecord StudentRecord_for_cat = K12.Data.Student.SelectByID(dr["學生系統編號"].ToString());

                            //基本上先預訂及格標準為60分

                            decimal Pre_Passing_Score = 0;

                            decimal Passing_Score = 60;
                            ...
                            foreach (XmlElement element in helper.GetElements("及格標準/學生類別"))
                            {
                                ...
                                //掃描學生的類別作比對
                                foreach (CategoryInfo catinfo in stuRec.StudentCategorys)
                                {
                                ...
                                if (cat == "預設" || useful)
                                {
                                    ...
                                    {
                                        if (stuRec.RefClass.GradeYear + "" == "1")
                                        {
                                            if (decimal.TryParse(element.GetAttribute("一年級及格標準"), out Pre_Passing_Score))
                                            {
                                                //擇優判斷
                                                if (Passing_Score > Pre_Passing_Score)
                                                {
                                                    Passing_Score = Pre_Passing_Score;
                                                }
                                            }
                                        }


                                        if (stuRec.RefClass.GradeYear + "" == "2")
                                        {
                                            //擇優判斷
                                            if (decimal.TryParse(element.GetAttribute("一年級及格標準"), out Pre_Passing_Score))
                                            {
                                        ...
                                        if (stuRec.RefClass.GradeYear + "" == "3")
                                        {
                                            //擇優判斷
                                            if (decimal.TryParse(element.GetAttribute("一年級及格標準"), out Pre_Passing_Score))
                                            {
                                            ...
                                            if (studentExamSores[studentID].ContainsKey(subjectName))
                                            {
                                                foreach (var courseID in studentExamSores[studentID][subjectName].Keys)
                                                {
                                                    ...
                                                    {//有輸入
                                                        ...
                                                        row["科目" + subjectIndex] = sceTakeRecord.Subject + GetNumber(subjectNumber);
                                                        row["學分" + subjectIndex] = sceTakeRecord.CreditDec();

                                                        row["成績" + subjectIndex] = sceTakeRecord.SpecialCase == "" ? ("" + sceTakeRecord.ExamScore) : sceTakeRecord.SpecialCase;

                                                        decimal subject_score ;

                                                        decimal.TryParse(row["成績" + subjectIndex]+"", out subject_score);



                                                        // 幫不及格者加上星號
                                                        if (subject_score < Passing_Score) 
                                                        {
                                                            row["成績" + subjectIndex] = row["成績" + subjectIndex] + "*";
                                                    ...
                                                    {//修課有該考試但沒有成績資料
                                                        ...
                                                        if (courseRecs.Count > 0)
                                                        {
                                                            ...
                                                            row["科目" + subjectIndex] = courseRec.Subject + GetNumber(subjectNumber);
                                                            row["學分" + subjectIndex] = courseRec.CreditDec();
                                                            row["成績" + subjectIndex] = "未輸入";
                                                        }
                                                    }
                                                    #endregion
                                                    #region 參考成績
                        ...
                        {
                            _wbStudentList.Worksheets[0].Cells[rowIdx, 0].PutValue(dr["班級簡稱"].ToString());
                        ...
                        foreach (DataRow dr in table.Rows)
                        {
                            ...
                            foreach (DataColumn dc in table.Columns)
                            {
                                if (eKeyValDict.ContainsKey(dc.Caption))
                                {
                                    ...
                                    if (dc.Caption == "全班名次")
                                    {
                                        data[eKeyValDict[dc.Caption]] = dr[dc.Caption] + "/" + dr["全班名次母數"];

                                    }

                                    //2016/10/24 穎驊註解，因文華先前與前系統廠商亞馨、郵局討論的格式是匯"次別"，
                                    //經過討論後，先暫時繼續使用此格式，因此這邊固定程式碼鎖死"試別" 與"次別"的轉換，
                                    //以後如需更改可以再更改。

                                    else if (dc.Caption == "次別")                                    
                                    {
                                        if (dr[dc.Caption]+"" == "第一次期中考") 
                                        {
                                            data[eKeyValDict[dc.Caption]] = "一";
                                        }
                                        if (dr[dc.Caption]+"" == "第二次期中考")
                                        {
                                            data[eKeyValDict[dc.Caption]] = "二";
                                        }
                                        if (dr[dc.Caption]+"" == "期末考")
                                        {
                                            data[eKeyValDict[dc.Caption]] = "三";
                                        }
                            ...
                            for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                            {
                                if (dr["科目" + subjectIndex].ToString() != "")
                                {
                                    data["科目" + subjectIndex] = dr["科目" + subjectIndex];
                                    data["學分" + subjectIndex] = dr["學分" + subjectIndex];
                                    //        data["前次成績" + subjectIndex] = dr["前次成績" + subjectIndex];


                                        data["成績" + subjectIndex] =  dr["成績" + subjectIndex];                                    
                                                                       
                                    //        data["成績" + subjectIndex] = dr["科目成績" + subjectIndex];
                                    data["班平均" + subjectIndex] = dr["班均標" + subjectIndex];
                                    data["班排名" + subjectIndex] = dr["班排名" + subjectIndex] + "/" + dr["班排名母數" + subjectIndex];
                                    //        data["科排名" + subjectIndex] = dr["科排名" + subjectIndex];
                                    //        data["類組排名" + subjectIndex] = dr["類別1排名" + subjectIndex];
                                }
                            }
